...
def gwo(train_feature,test_feature,train_label,test_label,SearchAgents_no,Max_iteration,dim,lb,ub):
    ...
    for l in range(0,Max_iteration):
        ...
        for i in range(0,(Positions.shape[0])):
            ...
            for j in range(0,(Positions.shape[1])): 
                ...
                if Flag4lb:                   
                    Positions[i,j] = lb
            #RF模型训练
            clf2 = svm.SVC(kernel = 'rbf', C = Positions[i][0], gamma = Positions[i][1]).fit(train_feature, train_label)  #svm的使用函数
            ...
            #RF模型预测及其精度
            cv_scores = model_selection.cross_val_score(clf2,test_feature,test_label,cv =5,scoring = 'accuracy')
        ...
        Convergence_curve[l]=Alpha_score/100
        print('----------------迭代次数--------------------' + str(index_iteration))
        print(Positions)
        print('C and gama:' + str(Alpha_pos))
        print('accuracy:' + str((100-Alpha_score)/100))
    
    cur = pd.DataFrame(Convergence_curve)
    cur.to_csv("Conver_curveSVM.csv")
    ...
    Bxy.to_csv('Best_Alpha_SVM.csv')
    searching_pos = pd.DataFrame(All_position)
    searching_pos.to_csv('All_Position_SVM.csv')
    print(Bxy)
    print(All_position)
...
def plot(iterations,accuracy):
    ...
    c.to_csv("GWO-SVM-Scurve.csv")
    plt.xlabel('Number of iteration',size = 20)
    plt.ylabel('accuracy',size = 20)
    plt.title('GWO_SVM parameter optimization')
    # plt.show()
    plt.savefig('GWO-SVM600dpi.png', dpi=600)
